...
//         model.addAttribute("targetURL", "AdminMemberList?pageNum=1");
   ...
   @GetMapping("AdminPolice")
   public String memberPolice(@RequestParam(defaultValue = "1") int pageNum, Model model,
                  @RequestParam(defaultValue ="") String searchKeyword,
                  @RequestParam(defaultValue = "5") int listLimit) {
      // -------------------------------------------------------------------------------------------
      // 페이징 처리
...
//      System.out.println(pageInfo);
      // -------------------------------------------------------------------------------------------
      return "admin/admin_member_police";
   }
   ...
   public String changeMemberAuthorize(@RequestParam(defaultValue = "1") String member_status, String member_id, String isPolice, Model model) {
      ...
      if(memberRegCount > 0) {
         model.addAttribute("msg", "성공적으로 처리되었습니다.");
         model.addAttribute("targetURL", "AdminPolice");
         return "result/success";
      }  else {
         model.addAttribute("msg", "권한 변경에 실패했습니다.");
//         model.addAttribute("targetURL", "AdminMemberList?pageNum=1");
