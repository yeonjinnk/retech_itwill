...
//	}
	public KakaoToken requestKakaoAccessToken(String code) {
		URI uri = UriComponentsBuilder
				.fromUriString(REQUEST_TOKEN_URL) // 요청 주소 설정
				.encode() // 주소 인코딩
				.build() // UriComponents 타입 객체 생성
...
//				new ParameterizedTypeReference<Map<String, String>>() {};
		...
		ResponseEntity<KakaoToken> responseEntity = 
				restTemplate.exchange(uri, HttpMethod.POST, httpEntity, KakaoToken.class);
	...
	public Map<String, Object> requestKakaoUserInfo(KakaoToken token) {
		URI uri = UriComponentsBuilder
